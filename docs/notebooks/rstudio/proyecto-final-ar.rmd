...
Escribe la fecha actual (**NOTE-01**): 3 de junio del 2021
...
```{r}
NOMBRE_COMPLETO = "Giovanna Díaz Medina"
DIGITOS_MATRICULA = 01375225
```
...
- Support del item {beer} (**NOTE-06**): 3/5
- Support del itemset {beer, nuts} (**NOTE-07**): 1/5
...
- ¿Cuál es la interpretación de la regla `{} => {diapers}` en la que se aprecia la ausencia del LHS? (**NOTE-19**): En la primera regla no hay una relación del producto pañales con otro de los artículos
...
- ¿Por qué no aparece listada la regla `{beer} => {nuts}`? (**NOTE-22**): Esto es debido a que las métricas de confidence y support excluyen a la regla mencionada
...
- ¿Cuál es el propósito de `session id`? (**NOTE-23**): Esto representa a cada una las sesiones del dataset, además de que su propósito es distinguir el número de sesiones que los usuarios tienen en Spotify para así poder hacer recomendaciones
- ¿Cuál es el propósito de `track id`? (**NOTE-24**): Esto representa a cada una de las canciones, distingue cuáles son las canciones que escuchan usuarios en específico y hacer recomendaciones con base en sus preferencias
- ¿Consideras que las otras columnas del dataset son aplicables para análisis por medio de reglas de asociación, justifica tu respuesta? (**NOTE-25**): Sí, esto se debe a que representan el comportamiento del usuario al acceder a Spotify y escuchar música. Además, se pueden hacer asociaciones si se entiende la relación entre canciones y los gustos de cada usuario por medio de las pausas realizadas al momento de escuchar música, las canciones saltadas, entre otras 
...
- ¿Cuál es el ID y frecuencia absoluta de la sesión más frecuente? (**NOTE-26**): t_bacf06d3-9185-4183-84ea-ff0db51475ce y 881
- ¿Cuántos items tiene el itemset con mayor número de ocurrencias en la lista element length distribution? (**NOTE-27**): itemset 20 con 1573 items
- En el contexto de este dataset con información de sesiones de streaming, ¿cuál es la interpretación de la media aritmética? (**NOTE-28**): Los usuarios tienen en promedio 14.61 sesiones 
...
- ¿Qué factores están asociados con que el algoritmo `apriori` con parámetros por omisión no haya producido ninguna regla de asociación, justifica tu respuesta? (**NOTE-29**): Ninguna de las reglas de asociación cumple con los parámetros que se establecieron bajo los valores de support y confidence
- ¿Qué riesgos identificas con la asignación de valores muy bajos en los parámetros de  `support` y `confidence` al invocar el algoritmo `apriori`? Que se pierda precisión en el análisis ya que se tendrá un mayor número de reglas de asociación que pueden ser menos certeras 
...
- Provee el enlace al video aquí: **NOTE-30** https://drive.google.com/drive/folders/1aimjpSNtzXfK8RYwMMH_4aryiBblrTY0?usp=sharing
